...
<body>
	...
	<link rel="stylesheet" type="text/css" href="/resources/css/heart.css">
...
for(var i=start-1;i<end;i++){
	...
	$('.bees-name-tag').eq(i).css('display','block');
...
if($(window).scrollTop()==($(document).height()-$(window).height())){
	...
	<script>
		...
		$(function(){
			...
			//사이드 네비 초기값
			$(".navi_li").eq(2).css('background','#F7D078').css('font-weight','700');
		...
		<div class="container pt-3">
			<div class="row">
				...
				<div class="col-7 p-0 myhistroy_div">
					...
					<table class="myhistroy_heart_table">
						<!-- 클릭하면 비즈로 이동 -->
			...
			if(!(feedList.isEmpty())){
				for (int i = 0; i < feedList.size(); i++) {
					...
					String beesName = feed.getBeesName();
				
				%>
			<div class="bees-name-tag"><a href="/beesSelectOne.do?beesNo=<%=feed.getBeesNo()%>&memberNo=${param.memberNo}" ><%=beesName %></a></div>
